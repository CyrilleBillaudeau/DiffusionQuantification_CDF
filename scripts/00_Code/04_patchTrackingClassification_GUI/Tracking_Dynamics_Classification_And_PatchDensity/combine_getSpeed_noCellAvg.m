function [resultPlot_velocity]=combine_getSpeed_noCellAvg(data_in,data_File,nFile,filename_label)

result_velocity=[data_File,data_in(:,4)];
result_velocity(data_in(:,3)~=1,:)=[];
...
if (nFile>1)
    ...
    resultPlot_velocity=[[resultPlot_velocity;NaN(size(result_velocity_all,1)-max(nVelocityPerFile),nFile)],result_velocity_all];
    nFilePlot=nFile+1;
else
    nFilePlot=nFile;
end%if
