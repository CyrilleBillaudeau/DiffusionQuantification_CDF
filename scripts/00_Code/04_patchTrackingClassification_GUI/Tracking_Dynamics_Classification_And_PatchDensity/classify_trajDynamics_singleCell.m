function [tab_patchDensityDyn,tab_patchDynamic,tabForBS]=classify_trajDynamics_singleCell(all_tr,traj2Cell,nCell,cellDescription,pixSize,lagTime,paramMSDanalysis,doPlot)
...
for iCell=1:nCell
    disp(strcat(['Cell #',num2str(iCell),' / ',num2str(nCell)]));
    if (cellDescription(iCell,7)>0)
        cell_area=cellDescription(iCell,4);
        cell_area=cell_area*(pixSize)^2;
        
        [tr,trajID,totalTraj,trajDuration,nFrame]=getTraj_CurrentCell(all_tr,traj2Cell,iCell);
        ...
        [tabDrift]=MSD_DriftModel(tr,totalTraj,minTrcLgth,pixSize,lagTime,doPlot);        
        % for bootstrap analysis
        startTrack=NaN(totalTraj,1);
        totalDisplacement_Track=NaN(totalTraj,2);
        for iTraj=1:totalTraj
            startTrack(iTraj,1)=min(tr(tr(:,4)==iTraj,3));
            curTraj=tr(tr(:,4)==iTraj,:);            
            totalDisplacement_Track(iTraj,1)=sqrt((curTraj(end,1)-curTraj(1,1))^2+(curTraj(end,2)-curTraj(1,2))^2)*pixSize;
            totalDisplacement_Track(iTraj,2)=sum(sqrt(sum(diff(curTraj(:,1:2)).^2,2)))*pixSize;
        end
        
        tabForBS=[tabForBS;iCell*ones(size(tabStatus)),trajID,tabStatus,tabSpeed,tabD,durTrack,startTrack,cell_area*ones(totalTraj,1),tabDrift,totalDisplacement_Track];
